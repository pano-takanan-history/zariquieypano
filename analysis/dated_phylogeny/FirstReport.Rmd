...
# Trees
tree <- read.beast('pano_conBDS.tree')
...
tree_pt <- ggtree(tree, aes(color=group), 
  ...
  geom_label(
    ...
    scale_x_continuous(breaks = c(), limits=c(-4670, 1100)) +
...
```{r}
tree_pt
ggsave('pano_conBDS.png', tree_pt, width=14, height=10)
```

Problematic traces: bone, count (verb), ear (!, saw this kind of trace before and this concept actually had the correct phylogeny), know (facts), pull, seed, throw, year