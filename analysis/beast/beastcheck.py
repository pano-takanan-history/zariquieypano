...
class BeastTest(object):
    ...
    tracelog = "$(filebase).log"
    treelog = "$(filebase).trees"
    ...
    def test_treelog(self):
        ...
        if expected not in [f.get('fileName') for f in treelogs]:
            raise AssertionError(f"Expected Tree log to be {expected} not {treelogs}")
    ...
    def test_tracelog(self):
        ...
        if log.get('fileName') != expected:
            raise AssertionError(f"Expected Trace log to be {expected} not {log.get('fileName')}")
...
class ModelGamma(object):
    ...
    def test_siteModel_gammaCategoryCount_gt_1(self):
        ...
        assert self.gammaCategoryCount == gammaCategoryCount, f"got {gammaCategoryCount} expected {self.gammaCategoryCount}"
...
class Analysis(AscertainmentBias, BirthDeathSkylineContemporaryBDSParamTreePrior, BeastTest):
    ...
    def test_BDSS_prior_birthRatePriorContempBDS_value(self):
        prior = self.xml.findall('./run/distribution/distribution/[@id="prior"]')[0]
        br = self.get_matching_children(prior, r"birthRatePriorContempBDS\.t:.*")[0]
        param = br.find('./Exponential/parameter')
        assert param.attrib['name'] == 'mean'
        assert param.text == '0.01'

    def test_BDSS_prior_deathRatePriorContempBDS_value(self):
        prior = self.xml.findall('./run/distribution/distribution/[@id="prior"]')[0]
        dr = self.get_matching_children(prior, r"deathRatePriorContempBDS\.t:.*")[0]
        param = dr.find('./Exponential/parameter')
        assert param.attrib['name'] == 'mean'
        assert param.text == '0.002'
...
class TestCTMCGamma4Strict(Analysis, ModelCTMC, ModelGamma, StrictClock, unittest.TestCase):
    filename = 'pano_ctmc_gamma4_strict.xml'

class TestCTMCGamma4Relaxed(Analysis, ModelCTMC, ModelGamma, OptimisedRelaxedClock, unittest.TestCase):
    filename = 'pano_ctmc_gamma4_relaxed.xml'

class TestCTMCGamma8Strict(Analysis, ModelCTMC, ModelGamma, StrictClock, unittest.TestCase):
    gammaCategoryCount = 8
    filename = 'pano_ctmc_gamma8_strict.xml'

class TestCTMCGamma8Relaxed(Analysis, ModelCTMC, ModelGamma, OptimisedRelaxedClock, unittest.TestCase):
    gammaCategoryCount = 8 
    filename = 'pano_ctmc_gamma8_relaxed.xml'
